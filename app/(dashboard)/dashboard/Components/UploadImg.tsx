...
const UploadImg = ({handleCallBack, id}:Props) => {
  const [data, setData] = useState<{image: string | null}>({image: null}); //{image: null}: Đây là giá trị khởi tạo ban đầu 
    ...
    <div>
      ...
      >
        <div className="absolute z-[5] h-full w-full rounded-md"
          onDragOver={(e) => { //onDragOver:sử dụng để xử lý khi người dùng kéo và thả (drag and drop) một phần tử lên một phần tử khác. 
            e.preventDefault();
            e.stopPropagation(); //ngăn chặn sự lan truyền (bubbling) của sự kiện
            setDragActive(true); //sự kiện kéo và thả đang diễn ra và đối tượng đang được kéo qua phần tử nà
          }}
          onDragEnter={(e) => {
            e.preventDefault();
            e.stopPropagation();
      ...
      <div className="mt-1 flex rounded-md shadow-sm">
        <input className="sr-only" id={id} name="image" type="file" accept="image/*"
